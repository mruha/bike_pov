...
BYTE rcv_spi (char dummy);		/* Send a 0xFF to the MMC and get the received byte (usi.S) */
...
DSTATUS disk_initialize (void)
{
	...
	//DESELECT();
	for (n = 10; n; n--) rcv_spi(1);	/* 80 dummy clocks with CS=H */

	ty = 0;
	if (send_cmd(CMD0, 0) == 1) {			/* Enter Idle state */
		if (send_cmd(CMD8, 0x1AA) == 1) {	/* SDv2 */
			for (n = 0; n < 4; n++) ocr[n] = rcv_spi(0);		/* Get trailing return value of R7 resp */
			if (ocr[2] == 0x01 && ocr[3] == 0xAA) {			/* The card can work at vdd range of 2.7-3.6V */
				for (tmr = 10000; tmr && send_cmd(ACMD41, 1UL << 30); tmr--) dly_100us();	/* Wait for leaving idle state (ACMD41 with HCS bit) */
				if (tmr && send_cmd(CMD58, 0) == 0) {		/* Check CCS bit in the OCR */
					for (n = 0; n < 4; n++) ocr[n] = rcv_spi(0);
...
)
{
	...
	if (buff) {		/* Send data bytes */
		...
		if (sa) {	/* Initiate sector write process */
			...
			if ((rcv_spi(0) & 0x1F) == 0x05) {	/* Receive data resp and wait for end of write process in timeout of 500ms */
				for (bc = 5000; rcv_spi(0) != 0xFF && bc; bc--) dly_100us();	/* Wait ready */
				if (bc) res = RES_OK;
			}
			//DESELECT();
			rcv_spi(1);
